let value: string | number; // string or number가 들어올 수 있음.
value = "hello";
value = 123;
...
// 사용자 타입 가드----------------------------------------
// 나만의 타입 (interface - 객체 / type - 모든 유형)
type Fish = {
  swim: () => void; // 아무것도 안하는 함수.
};
type Bird = {
  fly: () => void; // 아무것도 안하는 함수.
};
// function isFish(매개변수):animal is Fish{} - 함수의 결과가 true면 animal은 Fish false면 bird
function isFish(animal: Fish | Bird): animal is Fish {
  return (animal as Fish).swim !== undefined; // 만약 animal이 fish라면(가정) swim이 있고 이게 true면 undefined는 false 이니 결국 결과는 true
}
